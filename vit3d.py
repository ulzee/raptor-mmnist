...
class ViTMAEDecoderOutput(ModelOutput):
    ...
    Args:
        logits (`torch.FloatTensor` of shape `(batch_size, sequence_length, patch_size ** 3 * num_channels)`):
...
def get_3d_sincos_pos_embed(embed_dim, grid_size, add_cls_token=False):
    ...
    # print('  ', pos_embed.shape, embed_dim)
    if add_cls_token:
...
def get_3d_sincos_pos_embed_from_grid(embed_dim, grid):
    ...
    # use one third of dimensions to encode grid_d, grid_h, and grid_w
...
class ViTMAEEmbeddings(nn.Module):
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_3d_sincos_pos_embed(
            self.position_embeddings.shape[-1], int(round(self.patch_embeddings.num_patches**(1/3))), add_cls_token=True
        )
        if pos_embed.shape[0] != self.position_embeddings.shape[1]:
            raise ValueError(f"Position embedding size mismatch: expected {self.position_embeddings.shape[1]}, got {pos_embed.shape[0]}")
        self.position_embeddings.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))

        # initialize patch_embeddings like nn.Linear (instead of nn.Conv3d)
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor, depth: int, height: int, width: int) -> torch.Tensor:
        ...
        volumes. This method is also adapted to support torch.jit tracing.
        ...
        if not torch.jit.is_tracing() and num_patches == num_positions and depth == height == width:
        ...
        cube_num_positions = torch_int(num_positions**(1/3))
        patch_pos_embed = patch_pos_embed.reshape(1, cube_num_positions, cube_num_positions, cube_num_positions, dim)
...
class ViTMAEPatchEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        volume_size, patch_size = config.volume_size, config.patch_size
        num_channels, hidden_size = config.num_channels, config.hidden_size
        volume_size = volume_size if isinstance(volume_size, collections.abc.Iterable) else (volume_size, volume_size, volume_size)
        patch_size = patch_size if isinstance(patch_size, collections.abc.Iterable) else (patch_size, patch_size, patch_size)
        num_patches = (volume_size[2] // patch_size[2]) * (volume_size[1] // patch_size[1]) * (volume_size[0] // patch_size[0])
    ...
    def forward(self, pixel_values, interpolate_pos_encoding: bool = False):
        ...
        if not interpolate_pos_encoding and (depth != self.volume_size[0] or height != self.volume_size[1] or width != self.volume_size[2]):
            raise ValueError(
                f"Input volume size ({depth}*{height}*{width}) doesn't match model ({self.volume_size[0]}*{self.volume_size[1]}*{self.volume_size[2]})."
            )
...
class ViTMAEDecoder(nn.Module):
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor) -> torch.Tensor:
        ...
        patch_pos_embed = patch_pos_embed.reshape(1, 1, 1, -1, dim)
        ...
        patch_pos_embed = nn.functional.interpolate(
            patch_pos_embed,
            size=(patch_pos_embed.shape[-3], patch_pos_embed.shape[-2], embeddings_positions),
            mode="bicubic",
            align_corners=False,
        )
    ...
    def initialize_weights(self, num_patches):
        ...
        decoder_pos_embed = get_3d_sincos_pos_embed(
            self.decoder_pos_embed.shape[-1], int(round(num_patches**(1/3))), add_cls_token=True
        )
...
class ViTMAEForPreTraining(ViTMAEPreTrainedModel):
    ...
    def patchify(self, pixel_values, interpolate_pos_encoding: bool = False):
        ...
        patchified_pixel_values = pixel_values.reshape(
            batch_size, num_channels, num_patches_d, patch_size, num_patches_h, patch_size, num_patches_w, patch_size
        )
        patchified_pixel_values = torch.einsum("ncdrhpwq->ndrhpwqc", patchified_pixel_values)
        patchified_pixel_values = patchified_pixel_values.reshape(
            batch_size, num_patches_d * num_patches_h * num_patches_w, patch_size**3 * num_channels
        )
        return patchified_pixel_values

    def unpatchify(self, patchified_pixel_values, original_image_size: Optional[Tuple[int, int, int]] = None):
        """
        Args:
            patchified_pixel_values (`torch.FloatTensor` of shape `(batch_size, num_patches, patch_size**3 * num_channels)`:
                Patchified pixel values.
            original_image_size (`Tuple[int, int, int]`, *optional*):
                Original image size.
        ...
        if num_patches_d * num_patches_h * num_patches_w != patchified_pixel_values.shape[1]:
            raise ValueError(
                f"The number of patches in the patchified pixel values {patchified_pixel_values.shape[1]}, does not match the number of patches on original image {num_patches_d}*{num_patches_h}*{num_patches_w}"
            )
        ...
        patchified_pixel_values = torch.einsum("ndhwrpqc->ncdrhpwq", patchified_pixel_values)
    ...
    def forward_loss(self, pixel_values, pred, mask, interpolate_pos_encoding: bool = False):
        ...
        Args:
            pixel_values (`torch.FloatTensor` of shape `(batch_size, num_channels, depth, height, width)`):
                Pixel values.
            pred (`torch.FloatTensor` of shape `(batch_size, num_patches, patch_size**3 * num_channels)`:
    ...
    def forward(
        ...
        logits = decoder_outputs.logits  # shape (batch_size, num_patches, patch_size*patch_size*patch_size*num_channels)
